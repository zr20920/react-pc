...
enum ActionType {
  ...
  SET_ASSIGNMENT_CONSULT_MODAL = '[SetAssignmentConsultModal Action]',
}
...
const MyClue = () => {
  ...
  const assignmentConsultModalRef = useRef<LayoutFormModalCallType>()
  const [state, dispatch] = useReducer<ReducerType>((state, action) => {
    switch (action.type) {
      ...
      case ActionType.SET_ASSIGNMENT_CONSULT_MODAL: // 设置分配咨询弹窗
        return {
          ...state,
          assignmentConsultModal: {
            ...state.assignmentConsultModal,
            ...action.payload,
          },
        }
    }
  ...
  /**
   * @Description 设置分配咨询弹窗
   * @Author zhangrong
   * @Date 2020-09-19 16:35:38
   */
  const handleAssignmentConsultModalState = (
    data: Partial<StateType['assignmentConsultModal']>,
  ) => {
    dispatch({
      type: ActionType.SET_ASSIGNMENT_CONSULT_MODAL,
      payload: data,
    })
  }
  ...
  const handleBatch = (type: 'delete' | 'edit' | 'sendMsg' | 'allocation') => {
    if (mainListTableRef.current) {
      ...
      // 分配咨询
      if (type === 'allocation') {
        handleAssignmentConsultModalState({ visible: true })
      }
    }
  }
  ...
  /**
   * @Description 设置分配咨询表单
   * @Author zhangrong
   * @Date 2020-09-19 16:37:32
   */
  useEffect(() => {
    handleAssignmentConsultModalState({
      formList: [
        {
          componentName: 'Select',
          name: 'a',
          ...
          placeholder: '请选择电话销售',
          colProps: { span: 24 },
          rules: [{ required: true, message: '请选择电话销售' }],
          selectData: [],
        },
        {
          componentName: 'Select',
        ...
        },
        {
          ...
          placeholder: '请选择副销售员',
          colProps: { span: 24 },
          rules: [{ required: true, message: '请选择副销售员' }],
          selectData: [],
        },
      ],
    })
  ...
  return (
    <div className="mt-4">
      <LayoutTableList
        ...
        cardTopTitle={
          <Space size={10}>
            ...
            <Button>转到公共</Button>
            <Button onClick={() => handleBatch('allocation')}>分配咨询</Button>
            <Button>导入</Button>
            <Button>导出</Button>
          </Space>
        }
      ...
      />
      <LayoutFormModal
        ref={assignmentConsultModalRef}
        onCancel={() => handleAssignmentConsultModalState({ visible: false })}
        {...state.assignmentConsultModal}
      />
    </div>
  )
